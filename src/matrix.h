...
typedef double LIBICP_FLOAT;      // double precision
//typedef float  LIBICP_FLOAT;    // single precision
...
public:
  ...
  Matrix (const int32_t m,const int32_t n,const LIBICP_FLOAT* val_); // init mxn matrix with values from array 'val'
  ...
  void getData(LIBICP_FLOAT* val_,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);
  ...
  void setVal(LIBICP_FLOAT s,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);

  // set (part of) diagonal to scalar, -1 as end replaces whole diagonal
  void setDiag(LIBICP_FLOAT s,int32_t i1=0,int32_t i2=-1);
  ...
  static Matrix rotMatX(const LIBICP_FLOAT &angle);
  static Matrix rotMatY(const LIBICP_FLOAT &angle);
  static Matrix rotMatZ(const LIBICP_FLOAT &angle);
  ...
  Matrix  operator* (const LIBICP_FLOAT &s);  // multiply with scalar
  Matrix  operator/ (const Matrix &M); // divide elementwise by matrix (or vector)
  Matrix  operator/ (const LIBICP_FLOAT &s);  // divide by scalar
  Matrix  operator- ();                // negative matrix
  Matrix  operator~ ();                // transpose
  LIBICP_FLOAT   l2norm ();                   // euclidean norm (vectors) / frobenius norm (matrices)
  LIBICP_FLOAT   mean ();                     // mean of all elements in matrix
  ...
  LIBICP_FLOAT  det ();                                             // returns determinant of matrix
  bool   solve (const Matrix &M,LIBICP_FLOAT eps=1e-20);            // solve linear system M*x=B, replaces *this and M
  bool   lu(int32_t *idx, LIBICP_FLOAT &d, LIBICP_FLOAT eps=1e-20);        // replace *this by lower upper decomposition
...
private:
  ...
  void releaseMemory ();
  inline LIBICP_FLOAT pythag(LIBICP_FLOAT a,LIBICP_FLOAT b);
